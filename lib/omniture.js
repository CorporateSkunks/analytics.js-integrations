...
Omniture.prototype.getMapping = function(key) {

    var self = this;
    var mapping = self.options.mappings.events[key];

    // Keep a reference of dictionnary keys
    // TODO should this move into a dedicated prop?
    var dictionaryKeys = [];
    each(self.options.dictionary, function(key) {
        ...
        if(varValue.indexOf("{") !== -1) {
            ...
            mapping.variables[varName] = mapping.variables[varName].replace(/{([^}]+)}/g, function(str, p1, offset, s) {
                ...
                if(dictionaryKeys.indexOf(p1) !== -1) {
                    return self.options.dictionary[p1];
                } else {
...
Omniture.prototype.track = function(track, properties) {
    ...
    var mapping = this.getMapping(track.event());
